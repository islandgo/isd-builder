...
export class RegionsComponent {
    ...
    this.RegionsService.getDefaultRegions().subscribe((data) => {
      ...
      this.RegionsService.getRegionSetting().subscribe((region) => {
        ...
        this.defaultRegionsObject.forEach(function (data, i) {
          ...
          data[1]["background_setting"] =
            _this.RegionSectionModel.getBackGroundSettings(
              data[1]["keyword_class"]
            );
        });
      });
  ...
  private addCheckboxes() {
    Object.entries(this.defaultRegions).forEach(([key, value]) =>
      this.customRegionsIds.indexOf(value["region_id"]) !== -1
        ? this.defaultRegionsFormArray.push(new FormControl(true))
        : this.defaultRegionsFormArray.push(new FormControl(false))
    );
  }
  ...
  moveToCustomRegions() {
    ...
    this.RegionsService.setCustomRegions(jsonCustomRegions).subscribe(() =>
  ...
  editRegion() {
    this.defaultRegions;
    this.selectedRegion;
    this.defaultRegions[this.selectedRegion].description =
      this.capitalizeWord(this.newRegionName) + " Region";
    this.defaultRegions[this.selectedRegion].region_name =
      this.capitalizeWord(this.newRegionName) + " Region";
    ...
    this.defaultRegions[replacedKey.toUpperCase() + "_HOMEPAGE"] =
      this.defaultRegions[this.selectedRegion];
